...
st.markdown("<h3 style='text-align: left; color: yellow;'> Upload an image of a fish and the model will predict its species!</h3>", unsafe_allow_html=True)
...
@st.cache_resource
def load_model():
    model = tf.keras.models.load_model("Project 5-Multiclass Fish Image Classification/MobileNet/mobilenet_finetuned_model.h5")  # or cnn_fish_model.h5
    with open("Project 5-Multiclass Fish Image Classification/class_labels.json", "r") as f:
        labels = json.load(f)
    return model, labels
